...
public class CameraSource {
    ...
    private final or.kr.ajouhosp.stroke.GraphicOverlay graphicOverlay;
    ...
    public CameraSource(Activity activity, or.kr.ajouhosp.stroke.GraphicOverlay overlay) {
    ...
    /**
     * 카메라 시작 //CameraSourcePreview에서 호출
     */
    @RequiresPermission(Manifest.permission.CAMERA)
    public synchronized or.kr.ajouhosp.stroke.CameraSource start() throws IOException {
        if (camera != null) {
            return this;
        }

        camera = createCamera();
        dummySurfaceTexture = new SurfaceTexture(DUMMY_TEXTURE_NAME); //이미지 스트림의 프레임을 OpenGL ES 텍스처로 캡처합니다. 이미지 스트림은 카메라 미리보기 또는 비디오 디코딩에서 올 수 있습니다.
        camera.setPreviewTexture(dummySurfaceTexture); //프리뷰 내용을 텍스쳐로 캡쳐
        camera.startPreview(); //프리뷰 시작

        processingThread = new Thread(processingRunnable);
        processingRunnable.setActive(true);
        processingThread.start(); //쓰레드 시작
        return this;
    }

    //CameraSourcePreview에서 호출 : surfaceHolder 없는 경우
    @RequiresPermission(Manifest.permission.CAMERA)
    public synchronized or.kr.ajouhosp.stroke.CameraSource start(SurfaceHolder surfaceHolder) throws IOException { 
        if (camera != null) {
            return this;
        }
    ...
    private Camera createCamera() throws IOException {
        ...
        Camera camera = Camera.open(requestedCameraId); //카메라 열기
        ...
        camera.setPreviewCallbackWithBuffer(new CameraPreviewCallback());//버퍼를 여러개 사용해야 움직임이 부드럽다
        camera.addCallbackBuffer(createPreviewBuffer(previewSize)); //setPreviewCallbackWithBuffer를 실행하기 전에 3~4개의 버퍼를 넣어준다.
        camera.addCallbackBuffer(createPreviewBuffer(previewSize)); //setPreviewCallbackWithBuffer를 실행하고 필요한 위치에 startPreview한다
                ...
                try {
                    synchronized (processorLock) {
                        frameProcessor.processByteBuffer(
                                data,
                                new or.kr.ajouhosp.stroke.FrameMetadata.Builder()
